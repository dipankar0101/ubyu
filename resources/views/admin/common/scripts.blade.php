...
/***************/
$(document).on('click', '#addAttributeNew', function(e){
	var error = "";
	...
	if(error==""){
		...
		$("#loader").show();
		var formData = $('#addattributefromNew').serialize();
		$.ajax({
			url: '{{ URL::to("admin/products/attach/attribute/checkAttributeExist")}}',
			type: "POST",
			data: formData,
			success: function (res) {
				$("#loader").hide();
				...
				if(res == 'Yes'){
					...
					$('.addDefaultError').hide();
					$('#adddefaultattributesmodal').modal('hide');
					$('#addattributefromNew').submit();
				}
			},
		});
...
/***************/
$(document).on('click', '#adddefaultattributesmodalCheck', function(e){
	var products_id = $(this).attr('products_id');
	$.ajax({
		url: '{{ URL::to("admin/products/attach/attribute/checkProdOptionExist")}}',
		type: "POST",
		data: '&products_id='+products_id,
		success: function (res) {
			if(res == 'Yes'){
				$('.optionExistError').show();
			}else{
				$('.optionExistError').hide();
				$('#adddefaultattributesmodal').modal('show');
			}
		},
	});
});
...
$(document).on('click', '.editdefaultattributemodal', function(){
	var products_id = $(this).attr('products_id');
	var products_attributes_prices_id = $(this).attr('products_attributes_prices_id');

	$.ajax({
		url: "{{ URL::to('admin/products/attach/attribute/default/edit')}}",
		type: "POST",
		data: '&products_id='+products_id+'&products_attributes_prices_id='+products_attributes_prices_id,
		success: function (data) {
...
$(document).on('click', '#updateAddress', function(e){
	...
	/*****************************/
	$(document).on('click', '#updateDefaultAttributeNew', function(e){
		...
		if(error==""){
			...
			$("#loader").show();
			var formData = $('#editDefaultAttributeFromNew').serialize();
			$.ajax({
				url: '{{ URL::to("admin/products/attach/attribute/checkAttributeExistEdit")}}',
				type: "POST",
				data: formData,
				success: function (res) {
					$("#loader").hide();
					//console.log(res);
					if(res == 'Yes'){
						$('.EditDefaultError').show();
					}else{
						$('.EditDefaultError').hide();
						$('#editdefaultattributemodal').modal('hide');
						$('#editDefaultAttributeFromNew').submit();
					}
				},
			});
...
{{--{{ URL::to('admin/deleteproductattributemodal')}}--}}
	...
	$(document).on('click', '.deleteClassification', function(){
		$("#loader").show();
		var classifications_id = $(this).attr('classifications_id');
		$.ajax({
			url: "{{ URL::to('admin/classification/checkClassificationAssociate')}}",
			type: "POST",
			data: '&classifications_id='+classifications_id,
			success: function (res) {
				$("#loader").hide();
				if(res.length != ''){
					$('.addError').hide();
					...
					$('#classifications_id').val(classifications_id);
					$('#productListModal').modal('hide');
					$('#deleteattributeModal').modal('show');
					$(".contentAttribute").html(res);
				}
	...
	// delete-classification-value
	$(document).on('click', '.deleteClassificationValue', function(){
		$("#loader").show();
		var classification_values_id = $(this).attr('classification_values_id');
		var classifications_id = $(this).attr('classifications_id');

		$.ajax({
			url: "{{ URL::to('admin/classification/values/checkValueAssociate')}}",
			type: "POST",
			data: '&classification_values_id='+classification_values_id,
			success: function (res) {
				$("#loader").hide();
				//$('.deleteEmbed').html(res);
				//alert(res);
				if(res.length != ''){
					...
					$('#classification_values_id').val(classification_values_id);
					$('#classifications_id').val(classifications_id);
					$('#productListModalValue').modal('hide');
					$('#deleteValueModal').modal('show');
					$(".contentAttribute").html(res);
  ...
  $(document).on('click', '#deleteModelId', function(){
	...
	//deleteBulkDiscount
	$(document).on('click', '#deleteBulkDiscount', function(){
		var product_bulk_price_initial_id = $(this).attr('product_bulk_price_initial_id');
		$('#productBulkPriceInitialId').val(product_bulk_price_initial_id);
		$("#deleteBulkDiscountModal").modal('show');
	});

	//deleteshippingrate
	$(document).on('click', '#deleteshippingrate', function(){
		var shipping_rate_id = $(this).attr('shipping_rate_id');
		$('#shipping_rate_id').val(shipping_rate_id);
		$('#deleteshippingrateModal').modal('show');
	});
	//deleteshippingrate
	$(document).on('click', '#deleteYourImage', function(){
		var your_images_id = $(this).attr('your_images_id');
		$('#your_images_id').val(your_images_id);
		$('#deleteYourImageModal').modal('show');
	});
	//deleteproducfaq
	$(document).on('click', '#deleteproducfaq', function(){
		var products_faqs_id = $(this).attr('products_faqs_id');
		$('#products_faqs_id').val(products_faqs_id);
		$('#deleteProductsFaqsModal').modal('show');
	});

	//deletenewsletter
	$(document).on('click', '#deletenewsletter', function(){
		var news_letter_id = $(this).attr('news_letter_id');
		$('#news_letter_id').val(news_letter_id);
		$('#deleteNewsLetterModal').modal('show');
	});


	//delete common
	$(document).on('click', '#deleteCommon', function(){
		 var common_id = $(this).attr('common_id');
		 $('#id').val(common_id);
		 $('#deleteModalCommon').modal('show');
	});

	//deletebrand

	$(document).on('click', '#deletedirectory', function(){
		 var directory_id = $(this).attr('directory_id');
		 $('#directory_id').val(directory_id);
		 $('#deleteDirectoryModal').modal('show');
	});
    //delete_directory_details
    $(document).on('click', '#deletedirectorydetails', function(){
		 var directoriesDetail_id = $(this).attr('directoriesDetail_id');
		 $('#directoriesDetail_id').val(directoriesDetail_id);
		 $('#deleteDirectoryDetailsModal').modal('show');
	});
	//deleteclassification
	$(document).on('click', '#deleteclassification', function(){
		var classifications_id = $(this).attr('classifications_id');
	...
	//deletereturnreasons
	$(document).on('click', '#deletereturnreasons', function(){
		var return_reasons_id = $(this).attr('return_reasons_id');
		$('#return_reasons_id').val(return_reasons_id);
		$('#deleteReturnReasonsModal').modal('show');
	});
	//deletemeta
	...
	//deleteproductsmeta
	$(document).on('click', '#deleteproductsmeta', function(){
		var products_meta_id = $(this).attr('products_meta_id');
		$('#products_meta_id').val(products_meta_id);
		$('#deleteProductsMetaModal').modal('show');
	});
	// deletecontactusrequest
	$(document).on('click', '#deletecontactusrequest', function(){
		var contact_us_message_id = $(this).attr('contact_us_message_id');
		$('#contact_us_message_id').val(contact_us_message_id);
		$('#deleteContactUsRequestModal').modal('show');
	});
...
$(document).on('submit', '.form-validate-2', function(e){
	...
	//focus form field
	$(".stock-validate-2").each(function() {
		//var re = /^[-+]?[0-9]+\.[0-9]+$/;
		// /this.value.match( re )
		var x = event.keyCode;
		// if(this.value == '' || isNaN(this.value) || this.value > 0) {
...
$(document).on('click', '.required_one-2', function(e){
...
//focus form field
$(document).on('keyup', '.stock-validate-2', function(e){
	//var re = /^[-+]?[0-9]+\.[0-9]+$/;
	// /this.value.match( re )
	var x = event.keyCode;
	// if(this.value == '' || isNaN(this.value) || this.value > 0) {
...
function prodcust_type(){
    ...
    if(prodcust_type==2){
        ...
        $('#product_option_div').hide();
        $('#options_ids').parents('div.form-group').removeClass('has-error');
        $('#options_ids').removeClass('field-validate');
        ...
        $('#product_option_div').show();
        $('#options_ids').addClass('field-validate');
...
//Search
$( "#registration1" ).on('click','#submit',function( event ) {
...
//changeDirectoriesOrder
$(document).on('click','.changeDirectories',function(e){
	// var formData = $('.changeCategoriesOrder').serialize();
	var formData = $(this).closest('.changeDirectoriesOrder').serialize();
	$.ajax({
		url: '{{ URL::to("admin/directory/changeDirectoriesOrder")}}',
		type: "POST",
		data: formData,
		success: function (data) {
			// console.log(data);
...
$(document).on('click','.changeCrops',function(e){
	// var formData = $('.changeCategoriesOrder').serialize();
	var formData = $(this).closest('.changeCropsOrder').serialize();
	$.ajax({
		url: '{{ URL::to("admin/crop/changeCropsOrder")}}',
		type: "POST",
		data: formData,
		success: function (data) {
			// console.log(data);
